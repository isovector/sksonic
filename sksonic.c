...
void generate_subsonic_url (const Connection * conn, enum Operation operation,
...
void get_albums (const Connection * connection, Database * db,
		 const char *artist_id);
void get_songs (const Connection * connection, Database * db,
		const char *artist_id, const char *album_id);
...
static WINDOW **create_windows (const int n, const int bottom_space,
 ...
 */
static inline void
delete_windows (WINDOW *windows[], const int num_windows)
{
 ...
 */
static WINDOW **
create_windows (const int number_windows, const int bottom_space,
		const WindowType window_type)
{
 ...
 */
static int
get_character_byte_count (const char *text, const int max_width)
{
  const char *p = text;
 ...
 */
char *
format_text (const char *const text, const int max_width,
	     const char *const prefix)
{
  ...
  if (total_len <= max_width) {
    ...
    const size_t bytes_text = get_character_byte_count (text,
							max_width - (prefix ==
								     NULL ? 0
								     : 1));
 ...
 */
void
add_song (const Song *const song, Playlist *const playlist)
{
 ...
 */
void
delete_song (const AppState *const app_state)
{
 ...
 */
pid_t
get_pid (const Playback_Program program)
{
  char command[100];
 ...
 */
static inline void
change_playback_status (const pid_t pid, const int signal)
{
 ...
 */
void
dump (const AppState *const app_state)
{
 ...
 */
void
notify (const AppState *const app_state)
{
 ...
 */
void
play_song (const AppState *const app_state, const int index)
{
 ...
 */
void
stop_playback (const AppState *const app_state)
{
 ...
 */
void
pause_resume (const AppState *const app_state)
{
  Playlist *const playlist = app_state->playlist;
  time_t now;
 ...
 */
int
test_subsonic_connection (const Connection *const conn)
{
 ...
 */
char *
fetch_url_data (const char *const url)
{
 ...
 */
void
get_artists (const Connection *const connection, Database *db)
{
  ...
  cJSON_ArrayForEach (alphabetical_index, index) {
    ...
    cJSON_ArrayForEach (artist, artists_list) {
      ...
      a->id =
	strdup (cJSON_GetObjectItemCaseSensitive (artist, "id")->valuestring);
      a->name =
	strdup (cJSON_GetObjectItemCaseSensitive
		(artist, "name")->valuestring);
      a->number_albums = 0;
      a->albums = NULL;
      i++;
    }
 ...
 */
int
find_artist (const Database *const db, const char *const artist_id)
{
 ...
 */
int
find_album (const Artist *const artist, const char *const album_id)
{
  // Check if the artist is already in the database
 ...
 */
void
request_albums (const Connection *const conn, Artist *const artist)
{
  ...
  cJSON_ArrayForEach (album, albums) {
    ...
    a->name =
      strdup (cJSON_GetObjectItemCaseSensitive (album, "name")->valuestring);
    a->number_songs = 0;
    a->songs = NULL;
    i++;
  }
 ...
 */
void
get_albums (const Connection *conn, Database *db, const char *artist_id)
{
 ...
 */
int
approximate_duration (const cJSON *const song)
{
 ...
 */
void
request_songs (const Connection *conn, Album *album)
{
  if (album->songs != NULL) {
    // If album information for this artist has already been retrieved, return
    return;
  ...
  cJSON_ArrayForEach (song, songs) {
    ...
    s->name =
      strdup (cJSON_GetObjectItemCaseSensitive (song, "title")->valuestring);
 ...
 */
void
play_previous_next (AppState *const app_state, const int action)
{
 ...
 */
int
add_to_playlist (AppState *app_state)
{
  Playlist *playlist = app_state->playlist;
 ...
 */
void
print_progress_bar (WINDOW **windows, const AppState *const app_state)
{
 ...
 */
void
update_playlist_state (AppState *app_state)
{
  Playlist *const playlist = app_state->playlist;
 ...
 */
void
change_shuffle_repeat (Playlist *const playlist, const char action)
{
  if (playlist == NULL) {
    return;
  }
 ...
 */
void
handle_action (const int action, AppState *app_state)
{
  if (action == -1) {
    return;
  }
...
main (void)
{
  ...
  while (1) {
    ...
    case PLAYING:
	  ...
	  && playlist.play_time >= playlist.songs[playlist.current_playing]->duration) {
